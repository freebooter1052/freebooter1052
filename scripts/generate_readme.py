...
def generate_blog_content(medium_posts):
    ...
    return blog_posts_section.rstrip()
...
def generate_activity_content(github_activity):
    ...
    return activity_section.rstrip()
...
def replace_section(content, marker, new_text):
    ...
    return pattern.sub(lambda m: f"{m.group(1)}{new_text}\n{m.group(2)}", content)
...
def main():
    ...
    # Replace sections
    new_readme_content = replace_section(readme_content, "STATS", stats_content)
    new_readme_content = replace_section(new_readme_content, "BLOG", blog_content)
    new_readme_content = replace_section(new_readme_content, "ACTIVITY", activity_content)

    # Write to README.md if there are changes
