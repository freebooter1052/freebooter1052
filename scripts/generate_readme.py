...
def check_github_stats_api(username):
    url = f"https://github-readme-stats-fast.vercel.app/api?username={username}&show_icons=true&theme=dark&hide_border=true&bg_color=0d1117"
    try:
...
def generate_stats_content(github_username, leetcode_username, omit_github=False):
    github_stats_row = f"""
<div align="center">
  <table>
    <tr>
...
"""
    ...
    if omit_github:
        content = leetcode_stats_row.strip("\n")
    else:
        content = github_stats_row.strip("\n") + "\n\n" + leetcode_stats_row.strip("\n")
...
def main():
    ...
    github_api_broken = not check_github_stats_api(github_username)
    if github_api_broken:
        print("GitHub Stats API is broken or timed out. Omitting GitHub stats section.")

    stats_content = generate_stats_content(github_username, leetcode_username, omit_github=github_api_broken)
